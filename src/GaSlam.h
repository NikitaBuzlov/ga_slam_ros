  ...
  public:
   ...
   // GaSlam(ros::NodeHandle& Node_handle);
    ...
    /** Configures the managed submodules by passing the respective parameters
      ...
      *            equation of the depth sensor
      * @param[in] numParticles number         const double traversedDistanceThreshold = 10.;
        ...
        const double globalMapResolution = 1.;of particles used in the particle filter
   ...
   // void poseCallback(const Pose& odometryDeltaPose);
    void poseCallback(const nav_msgs::Odometry& odometryDeltaPose);
    ...
    /** Handles the input pose data from the IMU sensor. The orientation
      ...
      */
    void imuCallback(const sensor_msgs::Imu&/*const Pose&*/ imuOrientation);
