...
class PoseEstimation {
  public:
   ...
   /* tf::Transform getPose(void) const {
    ...
    void predictPose(const nav_msgs::Odometry& odo)
    {
        tf::Transform transform;
        Eigen::Affine3d pose;
        tf::poseMsgToTF(odo.pose.pose,transform);
    ...
    void fuseImuOrientation (const sensor_msgs::Imu& imuOrientation)
    {
        tf::Quaternion quat;
        tf::quaternionMsgToTF(imuOrientation.orientation, quat);
        Eigen::Quaterniond quat_e;
        tf::quaternionTFToEigen(quat,quat_e);
        Eigen::Affine3d pose(quat_e);
        fuseImuOrientation(pose);
    }
