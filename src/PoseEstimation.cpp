...
void PoseEstimation::fuseImuOrientation(const Pose& imuOrientation) {
   // const tf::Transform translation = getPose();
...
void PoseEstimation::filterPose(
    ...
    std::cout << " particle filter begin update" << rawCloud->size() << " size of raw data and map cloud " << mapCloud->size()<< std::endl;
    particleFilter_.update(lastPose, rawCloud, mapCloud);
    std::cout << " particle filter is updated" << std::endl;
